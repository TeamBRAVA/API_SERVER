...
do
	...
	openssl req -new -key device.key -out device.csr -passin pass:$passphrase -subj "/C=FR/ST=Ile-de-France/L=Paris/O=RED/CN=RED" &> /dev/null
	openssl x509 -req -days 36500 -in device.csr -CA ca.pem -CAkey ca.key -set_serial 01 -out device.crt -passin pass:$ca_passphrase &> /dev/null
	if ! [ -s "device.crt" ] ; then
		echo ""
   		echo "The specified CA KEY file passphrase is not correct." >&2
   		rm device.key &> /dev/null
   		rm device.csr &> /dev/null
   		rm device.crt &> /dev/null
   		exit 0
	...
	rm device.crt &> /dev/null
	rm device.p12 &> /dev/null
	mv device.pem $BASEDIR/certificates/$i.pem
	echo "#"$i $passphrase >> $BASEDIR/passphrases/passphrases.txt
	#echo "ok"
done
